...
import { MenuAction, MenuSelectedEvent } from './index-common';
...
function getIconForOption(option: MenuAction): UIImage | null {
  ...
  if ('fontFamily' in icon) {
    image = ImageSource.fromFontIconCodeSync(
      unescape(icon.text),
      new Font(icon.fontFamily, 24, null, icon.fontWeight ?? 400),
      makeColor(icon.color, option.destructive)
    ).ios;
  }
...
function buildMenu(options: Array<MenuAction> | MenuAction, onSelect: (option: MenuAction) => void): UIMenu {
  const rootMenu: MenuAction = Array.isArray(options)
    ? { name: '', icon: 'ellipsis', children: options, childrenStyle: 'inline' }
    : options;
